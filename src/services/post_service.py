...
class PostService:
    ...
    async def create_post(self, *, author_id: int, title: str, text: str) -> PostDTO:
        post = await self.post_repository.add_post(author_id=author_id, title=title, text=text)
    ...
    async def delete_post(self, *, author_id: int, post_id: int) -> None:
        if not await self.post_repository.delete_post(post_id=post_id, author_id=author_id):
            raise PostNotFound

    async def update_post(self, *, author_id: int, post_id: int, update_data: dict) -> PostDTO:
        post = await self.post_repository.update_post(post_id=post_id, author_id=author_id, update_data=update_data)
        if post:
