...
async def delete_comment(
    ...
    try:
        await comment_service.delete_comment(author_id=request.state.user.user_id, comment_id=comment_id)
...
async def update_comment(
        request: Request,
        comment_id: int,
        text: constr(strip_whitespace=True, min_length=5, max_length=500) = Body(embed=True),
