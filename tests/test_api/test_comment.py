...
async def test_create_comment(
    app: FastAPI, token: str, post_repository, create_data
) -> None:
    ...
    async with AsyncClient(
        ...
        response = await client.post("http://test/api/comments", json=create_data)
...
async def test_get_comment(app: FastAPI, token: str, post_repository, get_data) -> None:
    async with AsyncClient(
        ...
        response = await client.get(
            f"http://test/api/comments/{get_data['comment_id']}"
        )
...
async def test_update_comment(
    app: FastAPI, token: str, post_repository, update_data
) -> None:
    async with AsyncClient(
        app=app,
        base_url="http://test",
...
async def test_delete_comment(
    app: FastAPI, token: str, post_repository, delete_data
) -> None:
    async with AsyncClient(
        app=app,
        base_url="http://test",
